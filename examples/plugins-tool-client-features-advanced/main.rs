...
use plugin::ClientFeaturesAdvancedPlugin;
...
fn config_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("examples")
        .join("plugins-tool-client-features-advanced")
        .join("config")
        .join("mcp_config.yml")
}
...
async fn main() -> Result<(), McpError> {
    ...
    let plugins = PluginRegistry::new().register_tool(ClientFeaturesAdvancedPlugin)?;
