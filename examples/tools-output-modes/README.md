...
## Covers
- Structured output vs content mode rendering.

## Run
```bash
cargo run --example tools-output-modes
```
...
```
2) Call `notes.summary`:
```bash
curl -sS http://127.0.0.1:31943/mcp \
  -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"notes.summary","arguments":{"project_id":"p-1"}}}'
```
