...
## Covers
- HTTP tool with optional query parameter templating for web search.

## Run
```bash
cargo run --example tools-web-search
```
This example starts a local mock search upstream automatically.
...
curl -sS http://127.0.0.1:31943/mcp \
  ...
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"web_search","arguments":{"query":"mcp"}}}'
```
