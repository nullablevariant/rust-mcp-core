...
## Covers
- Client roots helper via `params.ctx.request_roots()` (`params: PluginCallParams`) from a tool plugin.

## Run
```bash
cargo run --example plugins-tool-client-features
```
...
Note: full `request_roots()` success requires a full MCP client that supports roots and can handle server-initiated requests.
