...
## Covers
- Inline + plugin prompt providers with prompt argument completions.

## Run
```bash
cargo run --example prompts-inline-plugin
...
curl -sS http://127.0.0.1:31943/mcp \
  ...
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d "{\"jsonrpc\":\"2.0\",\"id\":3,\"method\":\"prompts/get\",\"params\":{\"name\":\"summarize.document\",\"arguments\":{\"text\":\"hello\"}}}"
```
