...
## Covers
- Bearer-protected MCP endpoint with HTTP tool execution.

## Run
```bash
export MCP_BEARER_TOKEN="test-bearer-token"
cargo run --example auth-bearer
```
Value above is a test credential for local validation. This example starts a local mock upstream automatically.
...
```
`TOKEN` above is a test credential for this README. For this example, set `MCP_BEARER_TOKEN=test-bearer-token` when starting the server.
...
curl -sS http://127.0.0.1:31943/mcp \
  ...
  -H "authorization: Bearer ${TOKEN}" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"api.list\",\"arguments\":{\"query\":\"status\"}}}"
```
