...
## Covers
- Rich tool content blocks: text, image, audio, resource_link, and embedded resource.

## Run
```bash
cargo run --example tools-rich-content
...
```
2) Call `media.preview`:
```bash
curl -sS http://127.0.0.1:31943/mcp \
  -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"media.preview\",\"arguments\":{\"id\":\"media-1\"}}}"
```
