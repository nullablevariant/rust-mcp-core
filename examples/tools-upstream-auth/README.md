...
## Covers
- Upstream auth variants (`bearer`, `basic`, `oauth2`), per-upstream headers, and HTTP tool defaults.
- OAuth2 example includes test credentials (`test-partner-client-id`, `test-partner-client-secret`) for local demo wiring.

## Run
```bash
cargo run --example tools-upstream-auth
```
...
```
2) List registered tools (verifies config + auth variants loaded):
```bash
curl -sS http://127.0.0.1:31943/mcp \
  -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
...
```
Expected: response includes `reports.list`, `billing.get`, and `partner.list`.

3) Call `partner.list` to exercise outbound oauth2 token acquisition + bearer injection:
```bash
curl -sS http://127.0.0.1:31943/mcp \
  -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"partner.list","arguments":{"limit":2}}}'
```
