...
## Covers
- POST body templating with optional fields.

## Run
```bash
cargo run --example tools-http-post
```
...
```
2) Call `tickets.create`:
```bash
curl -sS http://127.0.0.1:31943/mcp \
  -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"tickets.create","arguments":{"title":"Bug report","labels":["p1"]}}}'
```
