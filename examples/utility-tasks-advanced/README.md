...
## Covers
- Required vs optional task support and full task lifecycle methods.

## Run
```bash
cargo run --example utility-tasks-advanced
```
...
```
2) Create required-task call:
```bash
curl -sS http://127.0.0.1:31943/mcp \
  -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"reports.generate","arguments":{"report_id":"r-1"},"task":{"ttl":60000}}}'
```
3) Optional tool can be called without task:
```bash
curl -sS http://127.0.0.1:31943/mcp \
  -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"reports.preview","arguments":{"report_id":"r-1"}}}'
```
