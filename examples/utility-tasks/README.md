...
## Covers
- Minimal task-augmented `tools/call` with `tasks/get` and `tasks/result`.

## Run
```bash
cargo run --example utility-tasks
```
...
```
3) Use returned `taskId` with:
```bash
curl -sS http://127.0.0.1:31943/mcp -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d '{"jsonrpc":"2.0","id":3,"method":"tasks/get","params":{"taskId":"<taskId>"}}'
curl -sS http://127.0.0.1:31943/mcp -H "content-type: application/json" \
  -H "accept: application/json, text/event-stream" \
  -H "MCP-Session-Id: ${SESSION_ID}" \
  -d '{"jsonrpc":"2.0","id":4,"method":"tasks/result","params":{"taskId":"<taskId>"}}'
```
